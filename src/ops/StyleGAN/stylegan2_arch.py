...
from src.ops.op import FusedLeakyReLU, fused_leaky_relu
from src.ops.op.upfirdn2d import upfirdn2d
...
from src.ops.StyleGAN.model import feature_modulation
        ...
        for conv1, conv2, noise1, noise2, to_rgb in zip(self.style_convs[::2], self.style_convs[1::2], noise[1::2],
            ...
            if cond_layers is not None:
                if i in cond_layers and conditions is not None:
                    ...
                    cond_weight = None
                    out = feature_modulation(out, condition, cond_weight, cond_type)
...
def visualize_feature_map(x):
    ...
    print(x.shape)
    b = x.shape[0]
    x_ = torch.max(abs(x), dim=1, keepdim=False)[0].cpu().detach().numpy()
    # x_ = torch.mean(x, dim=1, keepdim=False).cpu().detach().numpy()
    plt.figure()
    for i in range(b):
        plt.subplot(1, b, i+1)
